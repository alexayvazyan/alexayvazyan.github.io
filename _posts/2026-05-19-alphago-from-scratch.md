...
With a simple entropy addition into loss, and freezing the teacher policy every 10k steps, I was able to easily build a model that could beat me in 5x5 Go. It was time to scale up. From here on out, because I would no longer be evaluating the model's play myself (since it beats me reliably), there will be latent uncertainty about how good each model is. I can only say a model is better than others, but I will have no idea if its actually any good. The goal then is just to keep making better and better models that beat all their predecessors.
