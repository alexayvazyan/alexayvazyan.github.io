...
Imagine, in the Laplacean spirit, a description of the universe that loses nothing. Every particle's position and momentum, every field value, every quantum amplitude, at every instant. This representation is maximally faithful and maximally useless. It contains the answer to every question, but extracting any answer requires running the universe forward — which is what the universe is already doing. A representation that requires the system itself to evaluate has no leverage over the system.
...
Mathematics is interesting in this picture because it's the project of pushing as far up the tree (toward the root) as humans can manage while still preserving communicability. A theorem is an abstraction that retains so much structure that it generalizes across nearly all branches at lower depths. The price is that it takes a lot of training to use one, and even then most people can't.
